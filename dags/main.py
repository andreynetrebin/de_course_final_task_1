...
def read_file():
    ...
    if resource_info_response.status_code == 200:
        ...
        print(
            f'Ошибка при получении информации о ресурсе: {resource_info_response.status_code}, {resource_info_response.text}')
...
def process_data_and_validate():
    ...
    cleaned_df = df.filter(df["maintenance_year"].isNotNull() & (df["maintenance_year"] != "") & (
        df["maintenance_year"].cast(IntegerType()).isNotNull()))
    ...
    decade_counts = transformed_df.withColumn("Десятилетие",
                                              (F.floor(transformed_df["maintenance_year"] / 10) * 10).cast(
                                                  IntegerType())) \
